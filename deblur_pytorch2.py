...
def convert_to_tflite(model_path):
    # Load PyTorch model
    pytorch_model = mdl.UNet(n_channels=3, n_classes=3)
    pytorch_model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
    pytorch_model.eval()

    parent_path = model_path.split("/")[0]

    # Convert PyTorch model to ONNX
    dummy_input = torch.randn(1, 3, 256, 256)
    onnx_path = os.path.join(parent_path, 'model.onnx')
    ...
    # Optimize ONNX model
    passes = [
        'eliminate_identity', 'eliminate_nop_transpose', 'eliminate_unused_initializer',
        'eliminate_nop_pad', 'extract_constant_to_initializer', 'eliminate_nop_dropout'
    ]
    optimized_onnx_model = onnxoptimizer.optimize(onnx_model, passes)

    # Save the optimized ONNX model
    optimized_onnx_path = os.path.join(parent_path, 'optimized_model.onnx')
    onnx.save(optimized_onnx_model, optimized_onnx_path)

    # Convert ONNX model to TensorFlow
    tf_rep = prepare(optimized_onnx_model)
    tf_rep.export_graph(os.path.join(parent_path, 'model.pb'))

    # Convert TensorFlow model to TensorFlow Lite
    converter = tf.lite.TFLiteConverter.from_saved_model(os.path.join(parent_path, 'model.pb'))
    tflite_model = converter.convert()

    # Save the TensorFlow Lite model
    with open(os.path.join(parent_path, 'model.tflite'), 'wb') as f:
...
if __name__ == "__main__":
    ...
    if args.convert:
        model_path = "256_unet_epoch200/model_epoch_200.pth"
