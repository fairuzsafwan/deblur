...
class CustomDataset(tf.keras.utils.Sequence):
    def __init__(self, image_path_folder, batch_size=32, input_shape=(256, 256), shuffle=True):
        ...
        self.image_gt = os.listdir(os.path.join(image_path_folder, "sharp_v2"))
    ...
    def __getitem__(self, idx):
        ...
        for img_train, img_gt in zip(batch_image_train, batch_image_gt):
            ...
            gt_image_path = os.path.join(self.image_root_path, "sharp_v2", img_gt)
...
def trainModel(num_epochs=10, learning_rate=0.001, train_loader=None, model_path="./"):
    ...
    #GAN
    generator = mdl.UNet(n_channels=3, n_classes=3)
    discriminator = mdl.Discriminator()
    gan = mdl.GAN(generator=generator, discriminator=discriminator)
    ...
    # Assuming `dataset` is your dataset of real images
    ...
    def train_step(train_img, gt_img):
        with tf.GradientTape() as tape:
            ...
            pred_img = gan.train_step(train_img)
    ...
    for epoch in range(num_epochs):
        #total_loss = 0.0
        total_loss = {'d_loss': 0.0, 'g_loss': 0.0}
        num_batches = 0
        for batch_images, batch_gt_images in train_loader:
            #loss = train_step(batch_images, batch_gt_images)
            loss = gan.train_step(batch_images, batch_gt_images)
            #total_loss += loss
            total_loss['d_loss'] += loss['d_loss']
            total_loss['g_loss'] += loss['g_loss']
            num_batches += 1
        
        avg_d_loss = total_loss['d_loss'] / num_batches
        avg_g_loss = total_loss['g_loss'] / num_batches

        #print(f'Epoch [{epoch + 1}/{num_epochs}], Loss: {total_loss}')
        print(f'Epoch [{epoch + 1}/{num_epochs}], D Loss: {avg_d_loss}, G Loss: {avg_g_loss}')

        #model.save_weights(model_path_name)
        #model.save(os.path.join(model_path, str(epoch+1)))
        generator.save(os.path.join(model_path, str(epoch+1), "generator"))
        discriminator.save(os.path.join(model_path, str(epoch+1), "discriminator"))
